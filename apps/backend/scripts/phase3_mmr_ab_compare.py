#!/usr/bin/env python3
"""
Phase-3 A/B benchmark runner for MMR diversity policy.
...
def compare_reports(base_report: Dict[str, Any], cand_report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if agg_eps:
        for ep in agg_eps:
            ...
            q_cand += compare[ep]["mrr_mean"]["mmr"] + compare[ep]["ndcg_at_10_mean"]["mmr"]
    ...
    if agg_eps:
        for ep in agg_eps:
            ...
            lat_cand += compare[ep]["latency_p95_ms"]["mmr"]
    ...
    decision = {
        ...
        "latency_ceiling_pct": 6.0,
        "quality_pass": quality_delta_pct >= 0.0,
        "latency_pass": latency_delta_pct <= 6.0,
    }
    decision["recommend_mmr"] = bool(decision["quality_pass"] and decision["latency_pass"])

    return {
        "endpoint_compare": compare,
        "aggregate": {
            "quality_proxy_baseline": q_base,
            "quality_proxy_mmr": q_cand,
            "latency_p95_baseline_ms": lat_base,
            "latency_p95_mmr_ms": lat_cand,
        },
        "decision": decision,
    }
...
def write_compare_reports(
    ...
    json_path = report_dir / f"phase3_mmr_ab_compare_{ts}.json"
    md_path = report_dir / f"phase3_mmr_ab_compare_{ts}.md"

    payload = {
        "generated_at": datetime.now().isoformat(),
        "baseline_report_json": str(baseline_json),
        "mmr_report_json": str(mmr_json),
        **compare,
    }
    ...
    d = compare["decision"]
    lines = [
        f"# Phase-3 MMR A/B Compare Report ({ts})",
        "",
        "## Decision",
        f"- quality_delta_pct: {d['quality_delta_pct']:.2f}",
        f"- latency_delta_pct: {d['latency_delta_pct']:.2f}",
        f"- quality_pass (>= 0%): {d['quality_pass']}",
        f"- latency_pass (<= 6%): {d['latency_pass']}",
        f"- recommend_mmr: {d['recommend_mmr']}",
        "",
        "## Endpoint Deltas (mmr - baseline)",
    ]
...
def run_mode(
    ...
    env["SEARCH_MMR_ENABLED"] = "true" if mode == "mmr" else "false"
    env["SEARCH_MMR_LAMBDA"] = str(mmr_lambda)
    env["SEARCH_BM25PLUS_ENABLED"] = "true" if (mode == "mmr" and enable_bm25plus) else "false"
    env["SEARCH_RERANK_ENABLED"] = "true" if (mode == "mmr" and enable_rerank) else "false"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase-3 MMR A/B benchmark")
...
def main() -> int:
    ...
    print("[Phase3-AB] Running baseline...")
    ...
    baseline_copy = report_dir / f"phase3_mmr_baseline_{ts}.json"
    baseline_copy.write_text(baseline_json.read_text(encoding="utf-8"), encoding="utf-8")

    print("[Phase3-AB] Running candidate (mmr)...")
    mmr_json = run_mode(
        mode="mmr",
        py_exec=py_exec,
        backend_dir=backend_dir,
    ...
    mmr_copy = report_dir / f"phase3_mmr_candidate_{ts}.json"
    mmr_copy.write_text(mmr_json.read_text(encoding="utf-8"), encoding="utf-8")

    compare = compare_reports(
        json.loads(baseline_copy.read_text(encoding="utf-8")),
        json.loads(mmr_copy.read_text(encoding="utf-8")),
    )
    out_json, out_md = write_compare_reports(report_dir, ts, compare, baseline_copy, mmr_copy)

    print(f"[Phase3-AB] baseline:  {baseline_copy}")
    print(f"[Phase3-AB] candidate: {mmr_copy}")
    print(f"[Phase3-AB] compare:   {out_json}")
    print(f"[Phase3-AB] summary:   {out_md}")
    return 0
