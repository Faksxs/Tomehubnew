...
def _split_sql_blocks(sql_content: str) -> List[str]:
    """
    Split a SQL script into executable blocks using slash-delimited PL/SQL sections.
    """
    blocks: List[str] = []
    current: List[str] = []
...
def run_migration() -> int:
    migration_path = os.path.join(
        BACKEND_DIR,
        "migrations",
        "phaseX_oracle_text_content_ctx.sql",
    )
    ...
    try:
        ...
        conn.commit()
        print("Oracle Text migration applied successfully.")
        return 0
