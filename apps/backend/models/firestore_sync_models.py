...
class StrictFirestoreItem(BaseModel):
    ...
    # New parity columns from Frontend
    ...
    def entity_hash(self) -> str:
        data = {
            ...
            "langMode": self.contentLanguageMode,
            "langRes": self.contentLanguageResolved,
            "langHint": self.sourceLanguageHint,
            "langReason": self.languageDecisionReason,
            "langConf": self.languageDecisionConfidence,
            "folderId": self.personalFolderId,
...
def normalize_and_validate_item(item_id: str, raw: dict[str, Any]) -> StrictFirestoreItem:
    ...
    normalized = {
        ...
        "contentLanguageMode": str((raw or {}).get("contentLanguageMode") or "").strip()[:50] or None,
        "contentLanguageResolved": str((raw or {}).get("contentLanguageResolved") or "").strip()[:50] or None,
        "sourceLanguageHint": str((raw or {}).get("sourceLanguageHint") or "").strip()[:50] or None,
        "languageDecisionReason": str((raw or {}).get("languageDecisionReason") or "").strip()[:255] or None,
        "languageDecisionConfidence": _safe_float((raw or {}).get("languageDecisionConfidence")),
        "personalFolderId": str((raw or {}).get("personalFolderId") or "").strip()[:255] or None,
        "folderPath": str((raw or {}).get("folderPath") or "").strip()[:2000] or None,
    }
