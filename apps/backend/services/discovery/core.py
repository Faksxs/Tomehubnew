...
_DISCOVERY_SUMMARY_LLM_BACKOFF_SECONDS = 90.0
_discovery_summary_llm_disabled_until = 0.0

_TOKEN_RE = re.compile(r"[^\W_]+", re.UNICODE)
_DATE_RE = re.compile(r"\b(?:9\d{2}|1\d{3}|20\d{2})(?:[-/â€“]\d{2}(?:[-/â€“]\d{2})?)?\b")
...
_ENGLISH_HINT_RE = re.compile(r"\b(the|and|with|from|into|about|study|paper|work|published|author|research)\b", re.IGNORECASE)
...
def _build_family_cards(
    *,
    ...
    # Lazy imports to avoid circular dependency (providers import from core)
    if category_enum == DiscoveryCategory.ACADEMIC:
        from .academic_provider import _build_academic_cards
    ...
    if category_enum == DiscoveryCategory.RELIGIOUS:
        from .religious_provider import _build_religious_cards
    ...
    if category_enum == DiscoveryCategory.LITERARY:
        from .literary_provider import _build_literary_cards
        return _build_literary_cards(anchors, active_provider_names, selection_token=selection_token)
    from .culture_provider import _build_culture_history_cards
...
def _description_needs_enhancement(text: str, category: DiscoveryCategory) -> bool:
    ...
    if len(cleaned) < 60:
        return True
    if "|" in cleaned or "Type:" in cleaned or "DOI:" in cleaned:
        return True
    if category != DiscoveryCategory.RELIGIOUS and _ENGLISH_HINT_RE.search(cleaned):
        return True
    if category != DiscoveryCategory.RELIGIOUS and not _TURKISH_HINT_RE.search(cleaned) and cleaned.isascii():
        return True
    return False


def _enhance_card_description_llm(candidate_info: Dict[str, Any], category_name: str) -> str:
    global _discovery_summary_llm_disabled_until
    title = str(candidate_info.get("title") or "Unknown").strip()
    provider = str(candidate_info.get("provider") or "Source").strip()
    current_summary = str(candidate_info.get("summary") or "").strip()
    content = str(candidate_info.get("content_chunk") or "").strip()

    if time.monotonic() < _discovery_summary_llm_disabled_until:
        return current_summary

    prompt = f"""
    Create a concise Turkish description for this {category_name} discovery card.
    ...
    2. Use 2 short sentences, max 280 characters.
    3. Explain why the item matters based only on the provided data.
    4. Do not invent facts.

    Return ONLY the improved Turkish summary.
    """
    ...
    try:
        ...
        enhanced = str(result.text or "").strip()
        return enhanced or current_summary
    except Exception as exc:
        error_text = str(exc)
        if "429" in error_text or "RESOURCE_EXHAUSTED" in error_text or "quota" in error_text.lower():
            _discovery_summary_llm_disabled_until = time.monotonic() + _DISCOVERY_SUMMARY_LLM_BACKOFF_SECONDS
        logger.warning("discovery summary enhancement failed title=%s error=%s", title, exc)
        return current_summary
...
def _build_card_from_candidate(
    *,
    ...
    if parsed_date and parsed_type:
        freshness_label = f"{parsed_type} Â· {parsed_date}"
    elif parsed_date:
