...
def normalize_source_type(source_type: Optional[str]) -> str:
    ...
    Canonical set: PDF, EPUB, PDF_CHUNK, BOOK, ARTICLE, PERSONAL_NOTE, HIGHLIGHT, INSIGHT, MOVIE, SERIES
    """
    if not source_type:
        return "PERSONAL_NOTE"
    st = str(source_type).strip().upper()
    if st in {"PDF", "EPUB", "PDF_CHUNK", "BOOK", "ARTICLE", "PERSONAL_NOTE", "HIGHLIGHT", "INSIGHT", "MOVIE", "SERIES"}:
        return st
    if st == "BOOK_CHUNK":
        return "EPUB"
