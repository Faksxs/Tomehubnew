...
class LibraryServicePersonalNotesTests(unittest.TestCase):
    ...
    @patch("services.library_service.safe_read_clob", side_effect=lambda value: value)
    def test_bookmark_notes_fall_back_to_summary_when_not_indexed(
        self,
        _mock_safe_read_clob,
        _mock_library_cols,
        _mock_table_exists,
        ...
        fake_conn = _FakeConnection([[library_row], []])
        ...
        self.assertEqual(len(result["items"]), 1)
        note = result["items"][0]
        self.assertEqual(note["personalNoteCategory"], "BOOKMARK")
        self.assertEqual(note["generalNotes"], "<p>https://example.com</p>")
        self.assertEqual(note["highlights"], [])
