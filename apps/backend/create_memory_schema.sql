...
CREATE TABLE TOMEHUB_CHAT_SESSIONS (
    ...
    CONVERSATION_STATE_JSON CLOB CHECK (CONVERSATION_STATE_JSON IS JSON), -- Structured state (assumptions/facts/questions)
...
-- 4. Cross-session Memory Profiles (Always-on Memory MVP)
CREATE TABLE TOMEHUB_USER_MEMORY_PROFILES (
    ID NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    FIREBASE_UID VARCHAR2(255) NOT NULL,
    ...
    CREATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UPDATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uq_memory_profile_uid UNIQUE (FIREBASE_UID)
);

CREATE INDEX idx_memory_profile_uid ON TOMEHUB_USER_MEMORY_PROFILES(FIREBASE_UID);
