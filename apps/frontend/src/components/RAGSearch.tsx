        ...
        <div className="max-w-[1100px] w-full mx-auto p-6 space-y-6">
            ...
            <form onSubmit={handleSearch} className="space-y-4">
                ...
                <button
                    ...
                    className="w-full bg-[#CC561E] hover:bg-[#b34b1a] text-white font-medium py-3 px-6 rounded-xl transition-all shadow-lg shadow-[#CC561E]/20 flex items-center justify-center gap-2 disabled:bg-slate-300"
                >
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-slate-100 dark:border-slate-700">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex items-center gap-2">
                                    <button
                                        ...
                                        className={`p-2 rounded-lg transition-colors ${feedbackStatus === 'liked' ? 'bg-green-100 text-green-600' : 'text-slate-400 hover:text-green-600'}`}
                                    >
                                    ...
                                    <button
                                        ...
                                        disabled={feedbackStatus !== 'none' || isSubmittingFeedback}
                                        className={`p-2 rounded-lg transition-colors ${feedbackStatus === 'disliked' ? 'bg-red-100 text-red-600' : 'text-slate-400 hover:text-red-600'}`}
                                    >
                            ...
                            {result.metadata?.status === 'analytic' && result.metadata?.analytics?.contexts && result.metadata.analytics.contexts.length > 0 && (
                                <button
                                    ...
                                    className="flex items-center gap-2 text-[#CC561E] hover:text-[#b34b1a] font-bold text-sm transition-colors py-2 px-3 hover:bg-orange-50 dark:hover:bg-orange-900/10 rounded-lg"
                                >
                                ...
                                {result.sources.map((source, index) => (
                                    <div key={index} className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 transition-colors">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="font-medium text-slate-900 dark:text-white truncate">{source.title}</p>
                                            <p className="text-sm text-slate-600 dark:text-slate-400">Page {source.page_number}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
