...
import { Book, FileText, Globe, PenTool, StickyNote, BarChart2, X, Library, User } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange, isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside className={`
        fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 transform transition-transform duration-300 ease-in-out flex flex-col shadow-xl lg:shadow-none h-full
      ...
      `}>
        ...
        <div className="p-6 flex items-center justify-between h-20 border-b border-slate-50">
          <div className="flex items-center gap-3 text-indigo-600">
            ...
            <h1 className="text-xl font-bold text-slate-900 tracking-tight">TomeHub</h1>
          </div>
          <button onClick={onClose} className="lg:hidden p-1 hover:bg-slate-100 rounded-full text-slate-400 hover:text-slate-600 transition-colors">
            <X size={24} />
          </button>
        </div>
        ...
        <div className="p-3 border-t border-slate-100">
          ...
          >
            <User
              size={20}
              className={`transition-colors ${activeTab === 'PROFILE' ? 'text-indigo-600' : 'text-slate-400 group-hover:text-slate-500'}`}
            />
            Profile
          </button>
        </div>
        ...
        <div className="p-4 border-t border-slate-100">
          <div className="bg-slate-50/80 rounded-xl p-4 border border-slate-100">
            <p className="text-xs text-slate-500 font-medium mb-1">My Personal Library</p>
